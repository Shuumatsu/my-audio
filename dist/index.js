...
const audioDuration = audio => new Promise(resolve => {
  if (audio.duration) {
...
let AudioTemplate = class AudioTemplate extends HTMLElement {
  ...
  constructor() {
    ...
    this.playButtonHandler = () => this.status = 'playing';

    this.pauseButtonHandler = () => this.status = 'paused';

    this.stopButtonHandler = () => this.status = 'stopped';

    this.volumeInputHandler = () => this.volume = this.volumeInput.value / 100;
...
AudioTemplate.observedAttributes = ['src', 'volume', 'mode', 'show-lyrics', 'srcs', 'status'];
AudioTemplate.clickActions = ['playButton', 'pauseButton', 'stopButton'];
AudioTemplate.changeActions = ['volumeInput'];
...
AudioTemplate.observedAttributes.forEach(attribute => {
  Object.defineProperty(AudioTemplate.prototype, attribute, {
    ...
    set(val) {
      // nil check
      if (val == null) this.setAttribute(attribute);else this.setAttribute(attribute, val);
    }
  });
});
